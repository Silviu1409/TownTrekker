...
class VizualizareUrmareste: DialogFragment() {
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        if (refUser == mainActivityContext.getUser()!!.uid){
            adapter = VizualizareUrmaresteUrmaritoriAdapter(context, mainActivityContext.getUser()!!.urmareste)
            recyclerView.adapter = adapter
        }
        else {
            mainActivityContext.getDB().collection("useri").document(refUser).get()
                .addOnSuccessListener { doc ->
                    ...
                    val listaUrmareste = doc.get("urmareste") as? List<String> ?: listOf()
                    adapter = VizualizareUrmaresteUrmaritoriAdapter(context, listaUrmareste)
                    recyclerView.adapter = adapter
                }
        }
